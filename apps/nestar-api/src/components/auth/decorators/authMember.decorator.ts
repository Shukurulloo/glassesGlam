...
// custom decorator: u orqali guard ishga tushadi
export const AuthMember = createParamDecorator((data: string, context: ExecutionContext | any) => {
	...
	if (member) return data ? member?.[data] : member; // agar data bo'lsa talab etilgan datani qaytaradi aks holda member
	else return null;
});
